...
public sealed class UpdateArtworkTests
{
    ...
    public async Task WhenRequestIsNull_ReturnsBadRequest()
    {
        ...
        var result = await controller.UpdateArtwork(1, null!);
    ...
    public async Task WhenArtworkDoesNotExist_ReturnsNotFound()
    {
        ...
        var result = await controller.UpdateArtwork(1, request);
    ...
    public async Task WhenServiceReturnsNull_ReturnsInternalServerError()
    {
        ...
        artworkService.Setup(x => x.UpdateArtworkAsync(1, It.IsAny<UpdateArtworkDto>()))
    ...
    {
        var existingArtwork = new ArtworkDto { Id = 1, CreatorId = "creator-1", Name = "Old" };
        var updatedArtwork = new ArtworkDto { Id = 1, CreatorId = "creator-1", Name = "Updated" };

        var artworkService = new Mock<IArtworkService>();
        artworkService.Setup(x => x.GetArtworkByIdAsync(1, false)).ReturnsAsync(existingArtwork);
        artworkService.Setup(x => x.UpdateArtworkAsync(1, It.IsAny<UpdateArtworkDto>())).ReturnsAsync(updatedArtwork);
